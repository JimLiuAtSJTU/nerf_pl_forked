...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels,):
        ...
        self.activate = nn.LeakyReLU(negative_slope=1e-2)
...
using leaky relu to prevent the neuron from "dead"
...
class NeRF_Chromatic(nn.Module):
    def __init__(self,
        ...
        for i in range(D):
            if i == 0:
                ...
                layer = nn.Linear(W, W)
            layer = nn.Sequential(layer, nn.LeakyReLU(negative_slope=1e-2,inplace=True))
            setattr(self, f"xyz_encoding_{i+1}", layer)
        self.xyz_encoding_final = nn.Linear(W, W)
        ...
        self.rgb_residial_block_s=nn.Sequential(
            ResidualBlock(in_channels=W//2,out_channels=W//2),
            #ResidualBlock(in_channels=W // 2, out_channels=W // 2),
            #ResidualBlock(in_channels=W // 2, out_channels=W // 2)
        )
        '''
        self.rgb_residial_block2=nn.Sequential(
            ResidualBlock(in_channels=W//2,out_channels=W//2)
        #nn.Linear(W // 2, 3),
            #nn.ReLU();
        )
        
        self.rgb_residial_block3=nn.Sequential(
            ResidualBlock(in_channels=W//2,out_channels=W//2)
        #nn.Linear(W // 2, 3),
            #nn.ReLU();
        )
    ...
    def color_network(self,xyz_encoding_final,input_dir,input_chroma_code):
        ...
        y1 = self.dir_encoding(dir_encoding_input)
        y2 = self.rgb_residial_block_s(y1)
        #y3 = self.rgb_residial_block2(y2)
